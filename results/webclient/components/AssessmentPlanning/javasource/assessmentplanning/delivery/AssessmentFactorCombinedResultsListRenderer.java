...
/**
 * Renderer used for the assessment factor results list to display the results
 * for an assessment which contains a combination of role and group based
 * assessment questions.
 * 
 */
public class AssessmentFactorCombinedResultsListRenderer extends
  ...
  private static final String CLIENT = "Field.Label.Client";
  ...
  private static final String INDIVIDUAL = "Field.Label.Individual";
   ...
   */
  public AssessmentFactorCombinedResultsListRenderer() {

    properties =
      "assessmentplanning.delivery.AssessmentFactorCombinedResultsListRenderer";
  ...
  public void render(final Field field,
    ...
    final AssessmentFactorCombinedResultData assessmentFactorResultData =
      new AssessmentFactorCombinedResultData(myDoc);

    final Element outterDiv = createElement(documentFragment, "div");
    final Element groupDiv = createElement(documentFragment, "div");
    groupDiv.setAttribute("class", "list-no-space list");
    groupDiv.setAttribute("id", "priorityResultDiv");

    final String groupTitle =
      assessmentFactorResultData.getGroupAssessmentName();
    final Element groupTitleDiv = addTableTitle(documentFragment, groupTitle);

    groupDiv.appendChild(groupTitleDiv);

    final Element groupTable =
      documentFragment.getOwnerDocument().createElement("table");

    groupTable.setAttribute("cellspacing", "0");
    groupTable.setAttribute("cellpadding", "50");

    groupDiv.appendChild(groupTable);
    outterDiv.appendChild(groupDiv);

    final int numGroupColumnsToDisplay =
      addGroupTableColumns(documentFragment, assessmentFactorResultData,
        groupTable);

    addGroupTableHeader(documentFragment, context,
      assessmentFactorResultData, groupTable, numGroupColumnsToDisplay);

    addGroupTableBody(documentFragment, assessmentFactorResultData,
      groupTable, numGroupColumnsToDisplay);
      ...
      context.getDataAccessor()
        .get(
          ClientPaths.GENERAL_RESOURCES_PATH.extendPath(properties,
            INDIVIDUAL));
    final Element titleDiv = addTableTitle(documentFragment, title);
    ...
    documentFragment.appendChild(outterDiv);
  }

  /**
   * Adds the title to the table to be rendered.
   * 
   * @param documentFragment
   * The document fragment for the renderer
   * @param title
   * The title to be displayed on the results table
   * 
   * @return The div element where the title has been added.
   * 
   */
  private Element addTableTitle(final DocumentFragment documentFragment,
    final String title) {

    final Element titleDiv = createElement(documentFragment, "div");
    final Element titleH2 = createElement(documentFragment, "h2");
    final Element titleSpan = createElement(documentFragment, "span");
    titleDiv.setAttribute("class", "header-wrapper");
    titleH2.setAttribute("title", title);
    titleDiv.appendChild(titleH2);
    titleSpan.setAttribute("class", "collapse-title");
    titleH2.appendChild(titleSpan);
    final Node textElement =
      documentFragment.getOwnerDocument().createTextNode(title);
    titleSpan.appendChild(textElement);
    return titleDiv;
  }

  /**
   * Adds the body of the table to be rendered. This table contains the result
   * display for the group assessment question results.
   * 
   * @param documentFragment
   * The document fragment for the renderer
   ...
   */
  private void addGroupTableBody(final DocumentFragment documentFragment,
    final AssessmentFactorCombinedResultData assessmentFactorResultData,
    ...
    final List<FactorResultExt> factorResultsList =
      assessmentFactorResultData.getFactorGroupResultsList();
    ...
    for (final FactorResultExt factorResult : factorResultsList) {
      ...
      td1 = documentFragment.getOwnerDocument().createElement("td");
      setTDClass(colIndex, numColumnsToDisplay, td1);
      tr.appendChild(td1);
      colIndex++;
  ...
  private void addTableBody(final DocumentFragment documentFragment,
    ...
    final List<FactorResultExt> factorResultsList =
      assessmentFactorResultData.getFactorRoleResultsList();
  ...
  /**
   * Adds the header for the group table to be rendered.
   * 
   * @param documentFragment
   * The document fragment for the renderer
   * @param context
   ...
   * @throws DataAccessException
   */
  private void addGroupTableHeader(final DocumentFragment documentFragment,
   ...
   * {@link AssessmentFactorCombinedResultData#getClassificationResultLabelText()}
   * this is used as the label, else the default property value within the
   * property entry, 'Field.Label.Classification' is used.
   * 
   * @param context
   ...
   * this render through
   * {@link AssessmentFactorCombinedResultData#getFactorResultLabelText()} this
  ...
  /**
   * Adds the needed columns for the table which displays the group results The
   * number of columns to be displayed is returned.
   * 
   * @param documentFragment
   * The document fragment for the renderer
   * @param assessmentFactorResultData
   ...
   * @return The number of columns to be displayed
   */
  private int addGroupTableColumns(final DocumentFragment documentFragment,
    final AssessmentFactorCombinedResultData assessmentFactorResultData,
    final Element table) {
